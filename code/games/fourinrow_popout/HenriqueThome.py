...
class ThomePlayer(Player):
    ...
    def sucessores(self, player_code, board):
        ...
        for i in range(0,7):
            ...
            p = self.popmove(player_code, board, i)
            if(b is not None):
                suc.append({'board':b, 'action':i})
            if (p is not None):
                acao = "p"+str(i)
                suc.append({'board':p, 'action':acao})
        return suc
    ...
    def movement(self, player, board, column):
        result_board = np.matrix(board).copy()
    ...
    def popmove(self, player, board, column):
        if self.bottomDiscExist(player, board, column) == False:
            return None
        else:
            referencia = board.copy()
            y = board.copy()
            i = 0
            while(i<len(y)-1):
                y[i+1,column] = referencia[i,column]
                i+=1
            y[0,column] = 0
            return y

    def bottomDiscExist(self, player, board, column):
